...
function images() {
  return (
    src([config.paths.image + "/**/*", "!" + config.paths.image + "/sprite/*"])
      .pipe(config.isDev ? changed(config.pathsDev.html) : gutil.noop())
      // .pipe(imagemin({
      //     progressive: true,
...
// copy css
function copycss() {
  return src([config.paths.html + "/lib/*.css"]).pipe(dest(config.pathsDev.css));
}
exports.copycss = copycss;

// copy font
function copyfont() {
  return src([config.paths.html + "/font/*.*"]).pipe(dest(config.pathsDev.font));
}
exports.copyfont = copyfont;

// copy js
function copyjs() {
  return src([config.paths.html + "/lib/*.js"]).pipe(dest(config.pathsDev.script));
}
...
function watchList() {
  ...
  watch(config.paths.html + "/font/*.*", series(copyfont));
...
// init/build
exports.init = series(clean, html, styles, images, scripts, copycss, copyfont, copyjs, done => {
